...
const captures = [
  ...
  ["#panel", "chapter_panel_main_v3.png"],
  ...
  ["#example_detail_aerodynamics_panel", "example_detail_aerodynamics_panel.png"],
  ...
  ...STORY_UI_LAYOUT.steps.map((step) => step.id).flatMap((id) => (
    ["normal", "active", "pressed"].map((state) => [`#card_${id}_${state}`, `card_${id}_main_v3_${state}.png`])
  )),
  ...STORY_UI_LAYOUT.theoryCards.map((card) => card.id).flatMap((id) => (
    ["normal", "active", "pressed"].map((state) => [`#card_${id}_${state}`, `card_${id}_${state}.png`])
  )),
...
function buildLayoutTs(spec) {
  ...
  const theoryCards = spec.theoryCards.map((card) => {
    const slot = spec.slots.find((s) => s.id === `card_${card.id}`);
    return {
      id: card.id,
      index: card.index,
      title: card.title,
  ...
  const magneticModes = ["example_mode_magnetic_a", "example_mode_magnetic_b"].map((id) => spec.slots.find((s) => s.id === id).cm);
