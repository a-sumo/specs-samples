...
export class ColorSpacePlaneController extends BaseScriptComponent {
  ...
  /** Align pigment gamut with projector (tween to projector's current position/rotation/scale) */
  public alignPigmentWithProjector(duration: number = 0.5): void {
    if (this.pigmentMixGenerator && this.projectorGamutMesh) {
      ...
      const projScale = projTransform.getWorldScale();
      this.pigmentMixGenerator.tweenToTransform(projPos, projRot, duration, projScale);
    }
  }

  /** Align projector with pigment gamut (tween to pigment's current position/rotation/scale) */
  public alignProjectorWithPigment(duration: number = 0.5): void {
    if (this.pigmentMixGenerator && this.projectorGamutMesh) {
      ...
      const pigScale = pigTransform.getWorldScale();
      this.projectorGamutMesh.tweenToTransform(pigPos, pigRot, duration, pigScale);
    }
  }
}
