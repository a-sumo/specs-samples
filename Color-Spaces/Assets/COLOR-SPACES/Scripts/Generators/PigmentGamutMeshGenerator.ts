...
export class PigmentGamutMeshGenerator extends BaseScriptComponent {
  ...
  private transformTweenStartScale: vec3 = new vec3(1, 1, 1);
  private transformTweenEndScale: vec3 = new vec3(1, 1, 1);
  ...
  private storedRestScale: vec3 = new vec3(1, 1, 1);
  ...
  private captureRestTransform(): void {
    ...
    if (this.useCurrentAsRest) {
      ...
      this.storedRestScale = transform.getWorldScale();
    } else {
  ...
  private onUpdate(): void {
    ...
    if (this.isTransformTweening) {
      ...
      if (this.transformTweenElapsed >= this.transformTweenDuration) {
        ...
        transform.setWorldScale(this.transformTweenEndScale);
        ...
        const scale = vec3.lerp(this.transformTweenStartScale, this.transformTweenEndScale, eased);
  ...
  /** Tween SceneObject to rest transform (position + rotation + scale) */
  public tweenToRestTransform(duration: number = 0.5): void {
    ...
    this.transformTweenStartScale = transform.getWorldScale();
    this.transformTweenEndPos = this.storedRestPosition;
    this.transformTweenEndRot = this.storedRestRotation;
    this.transformTweenEndScale = this.storedRestScale;
  ...
  public snapToRestTransform(): void {
    ...
    transform.setWorldScale(this.storedRestScale);
    this.isTransformTweening = false;
  }

  /** Tween SceneObject to a specific world position/rotation/scale */
  public tweenToTransform(position: vec3, rotation: quat, duration: number = 0.5, scale?: vec3): void {
    this.ensureUpdateEvent();
    const transform = this.sceneObject.getTransform();
    this.transformTweenStartPos = transform.getWorldPosition();
    this.transformTweenStartRot = transform.getWorldRotation();
    ...
    this.transformTweenEndScale = scale ?? transform.getWorldScale();
    this.transformTweenDuration = duration;
    this.transformTweenElapsed = 0;
    this.isTransformTweening = true;
  }
  ...
  /** Get stored rest scale */
  public getRestScale(): vec3 {
    return this.storedRestScale;
  }

  /** Update rest transform to current position/rotation/scale */
