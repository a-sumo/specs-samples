// ColorSpaceTransform - Vertex position modifier for color space transformation
//
// The mesh is generated in RGB space. This shader:
// 1. Reads RGB from texture coords (the "center" color of each tube segment)
// 2. Calculates where that RGB center should be in the target color space
// 3. Computes the offset (vertex pos - RGB center pos)
// 4. Outputs: new_center + offset
//
// texture0.xy = r, g
// texture1.x = b
...
void main() {
    // Get RGB from texture coordinates (this is the "center" RGB for this vertex)
    ...
    // Current vertex position (object space, generated in RGB space)
    vec3 vertexPos = system.getSurfacePositionObjectSpace();

    // Where the RGB center is in RGB space
    vec3 rgbCenter = rgbSpacePosition(r, g, b, cubeSize);

    // Calculate positions in both color spaces
    ...
    // Vertex offset from center (tube radius, etc.)
